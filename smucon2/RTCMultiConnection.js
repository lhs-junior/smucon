...
(function() {
    ...
    function RTCMultiConnection(roomid, forceOptions) {
      ...
      //nextpage 화면 변경 후 값 넘겨 주기
        connection.nextpa= function(room_num) {
        	
        	alert("간다?");
        	location.href='/Test_views';
        	var openid = room_num;
        ...
        //roomlist 데이터베이스 목록 요청 
        connection.roomopenlist= function() {
        
        	socket.emit('db-roomlist');
        };
        
      //roomlist 데이터베이스 목록 요청 
        connection.roomIdupload= function(dbdata) {
        	var openid = dbdata;
        	socket.emit('db-store', { 
				openid : openid			
			});
        };
    ...
    function SocketConnection(connection, connectCallback) {
      ...
      // 서버에서 room-id값을 받아 페이지 넘기기
        socket.on('nextp',function(e) {
        	alert("ㅎㅇ");
        	console.log("no");
        	openrooms(e);
        });
        ...
        //DB에서 roomlist 받음
        socket.on('get-roomlist', function(getInfo) {
        	//var a = JSON.parse(getInfo);
        	//alert(het);
        	printroomlist(getInfo);
        });
        
      //DB에서 roomlid 중복시 출력
        socket.on('roomid-overlap', function(checks) {
        	if(checks==0)
        	{
        		openrooms();
        	}
